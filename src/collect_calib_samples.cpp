...
public:
        ...
        : nh_(nh), has_image_(false), saved_count_(0), mouse_clicked_(false)
    {
        ...
        nh_.param<double>("MaxBlur", max_blur_, 150.0);
...
private:
    ...
    void imageCallback(const sensor_msgs::ImageConstPtr& msg)
    {
        try
        {
            ...
            current_image_ = cv_ptr->image.clone(); // clone the image 
            has_image_ = true;
        }
    ...
    void saveImage()
    {
        ...
        cv::Point2f center = cv::Point2f(current_image_.cols/2.0f, current_image_.rows/2.0f);
        int q_idx = 0;
    ...
    void processAndDisplay()
    {
        ...
        // for (size_t i=0; i<quadrants_.size(); ++i) {
        //     cv::Scalar color = (quadrants_[i].saved_count >= target_per_quadrant_) ? cv::Scalar(0,150,0) : cv::Scalar(255,255,255);
        //     cv::rectangle(img, quadrants_[i].rect, color, 2);

        //     std::ostringstream label;
        //     label << "Q" << i+1 << ": " << quadrants_[i].saved_count << "/" << target_per_quadrant_;
        //     cv::putText(img, label.str(), quadrants_[i].rect.tl() + cv::Point(5,20),
